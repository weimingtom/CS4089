...
Kanade's face identification system was the first system in which all
...
face, which is called an $ \textit{eigenface}^{[\ref{fig:meigenface}]} $. \\
...
\begin{description}
  ...
  A value suggested by OpenCV documebtation is \textbf{8} sample points. \\
  ...
  \textbf{8} is a common value used in publications. \\
...
In the prototype implemented using the $ LBPH Approach^{[\ref{fig:lbph}]} $, calculation of the
...
\section{Conclusion}
In this project, I have tried to implement a Face Recognition based biometric
security system. It also demonstrates the various capabilities of Image Recognition
from video sequences. However, several aspects remain to be researched and extended.
