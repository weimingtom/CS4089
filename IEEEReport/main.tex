...
\begin{IEEEkeywords}
CMUSphinx, OpenCV, Real time, biometric, Face Recognition, PCA, Eigen faces,
...
Kanade's \cite{Kanade1973} face identification system was the first system in which all
steps of the recognition process were automated, using a top-down control strategy
directed by a generic model of expected feature characterstics. His system calculated
...
face, which is called an \textit{eigenface}. \\
The approach to face recognition using the eigenface approach involves the
following initiation operations: \\
\begin{enumerate}
  \item Acquire an initial set of characterstic face images (the training set). \\
...
\begin{enumerate}
  ...
  \item If the same unknown face is seen several times, calculate its characterstic
        weight pattern and incorporate into the known faces.
\end{enumerate}

<Insert the image here >
\\
In the prototype implemented, calculation of the eigenfaces is done as part of the
training process. \\
The recognition currently takes about 30 milli seconds implemented in
Python on an Intel Core i5, using face images of size 132 x 132.

\section{Work Plan for Next Semester}
\begin{enumerate}
  \item Develop an application which takes the voice input through the microphone,
        and train the application with multiple users.
  \item Modify the above application to test that user's voice and
